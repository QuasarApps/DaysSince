...
/**
 * On-device tests for [ElapsedTime], the elapsed-time core that every screen and widget renders from.
 *
...
class ElapsedTimeInstrumentedTest {
    ...
    fun sincePickedDhm_breaksElapsedIntoDaysHoursMinutes() {
        ...
        val dhm = ElapsedTime.sincePickedDhm(picked, pickedTime, clockAt(now), utc)
    ...
    fun sincePicked_returnsWholeDaysOnly() {
        val days = ElapsedTime.sincePicked(
    ...
    fun secondsAreIgnored_minutesAreFloored() {
        ...
        val dhm = ElapsedTime.sincePickedDhm(
            LocalDate.of(2025, 1, 1), LocalTime.of(0, 0), clockAt(now), utc,
        )

        assertEquals(ElapsedTime.ElapsedDhm(0, 0, 0), dhm)
    }
    ...
    fun springForwardDst_isAccountedFor_notAssumed24hDays() {
        ...
        val dhm = ElapsedTime.sincePickedDhm(picked, pickedTime, clockAt(now, london), london)
